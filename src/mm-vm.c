...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  if (old_sbrk + size < cur_vma->sbrk) {
    struct vm_rg_struct *r_rg = malloc(sizeof(struct vm_rg_struct));
    r_rg->rg_start = old_sbrk + size;
    r_rg->rg_end = cur_vma->sbrk;
    enlist_vm_freerg_list(caller->mm, r_rg);
  }
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller) {
  ...
  if (!PAGING_PAGE_PRESENT(
    ...
    /* Update page table */
    for (int i = 0; i< PAGING_MEMSWPSZ; i++)
    {
    	if (mm->pgd[i]==(vicpgn|PAGING_PTE_PRESENT_MASK)) {
    		mm->pgd[i] = swpfpn;
    		break;
    	}
    }
    MEMPHY_put_freefp(caller->active_mswp,tgtfpn);
    mm->pgd[pgn] = vicpgn|PAGING_PTE_PRESENT_MASK;
    // pte_set_swap() &mm->pgd;
  ...
  //*fpn = PAGING_FPN(pte);
  *fpn = pte & (~PAGING_PTE_PRESENT_MASK);
...

{
  ...
  if(currg->rg_start + offset >= currg->rg_end)
  {
     ...
     if (currg->rg_start + offset < currg->rg_end)   // MODIFIED BY STUDENT
...
int pgwrite(struct pcb_t *proc,   // Process executing the instruction
  ...
  printf("write region=%d offset=%d value=%d\n", destination, offset, data);
	...
	if (__write(proc, 0, destination, offset, data)==0){
		#ifdef PAGETBL_DUMP
		  print_pgtbl(proc, 0, -1); // print max TBL
		#endif
		#ifdef IODUMP
...
int find_victim_page(struct mm_struct *mm, int *retpgn) {
    ...
    // Set the return page number
    *retpgn = pg->pgn;
    // Update the FIFO page list to point to the next page
     if (prepg){
      prepg->pg_next = NULL;
    }
    else{
    ...
    // Free the memory allocated for the victim page
    free(pg);

    return 0; 
  /* TODO: Implement the theorical mechanism to find the victim page */
  // the oldest page as the victim page in FIFO
}
